...
for key, value in formal_covid_dict.items():
  for image_name, info in value['image_dict'].items():
    if 'PA' in info['type'] or 'AP' in info['type']:
      ...
      if value['class']['non_normal'] == 1:
          case_list.append((info['path'], key+'_'+image_name, 1))
      z0 += 1
print(len(case_list))

z1 = 0
formal_kaggle_dict = pickle.load(open('./data_preprocess/normal_non_normal_kaggle_dict.pkl','rb'))
...
exp_data_id = 'standard_data_multiclass_nnn'
...
with open(os.path.join(exp_data_dir, 'data_statistic.csv'),'w') as f:
  ...
  csv_writer.writerow(['N of Train', len(train_list), 'normal:%d'%train_s[0], 'non_normal:%d'%train_s[1]])
  csv_writer.writerow(['N of Valid', len(valid_list), 'normal:%d'%valid_s[0], 'non_normal:%d'%valid_s[1]])
  csv_writer.writerow(['N of Test', len(test_list), 'normal:%d'%test_s[0], 'non_normal:%d'%test_s[1]])
