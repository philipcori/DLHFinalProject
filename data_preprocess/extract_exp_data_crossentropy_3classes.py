...
exp_data_id = '3classes_data_multiclass_crossentropy'
...
for index, (train_list, valid_list) in enumerate(zip(train_data,valid_data)):
  ...
  with open(os.path.join(exp_data_dir, 'data_statistic.csv'),'w') as f:
    ...
    csv_writer.writerow(['N of Train', len(train_list), 'covid:%d' % train_s[0], 'pneumonia_virus:%d' % train_s[1],
                         'pneumonia_bacteria:%d' % train_s[2]])
    csv_writer.writerow(['N of Valid', len(valid_list), 'covid:%d' % valid_s[0], 'pneumonia_virus:%d' % valid_s[1],
                         'pneumonia_bacteria:%d' % valid_s[2]])
    csv_writer.writerow(['N of Test', len(test_list), 'covid:%d' % test_s[0], 'pneumonia_virus:%d' % test_s[1],
                         'pneumonia_bacteria:%d' % test_s[2]])
