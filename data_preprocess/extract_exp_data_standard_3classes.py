...
case_list = []
z0 = 0
formal_covid_dict = pickle.load(open('./data_preprocess/3classes_covid_dict.pkl','rb'))
...
z1 = 0
formal_kaggle_dict = pickle.load(open('./data_preprocess/3classes_kaggle_dict.pkl','rb'))
...
exp_data_id = 'standard_data_multiclass_3classes'
...
for x in test_list:
  test_s[x[2]] += 1
print (train_s)
# print ('N of Train', len(train_list), 'normal:%d'%train_s[0], 'non_normal:%d'%train_s[1])
...
with open(os.path.join(exp_data_dir, 'data_statistic.csv'),'w') as f:
  ...
  csv_writer.writerow(['N of Train', len(train_list), 'covid:%d'%train_s[0], 'pneumonia_virus:%d'%train_s[1], 'pneumonia_bacteria:%d'%train_s[2]])
  csv_writer.writerow(['N of Valid', len(valid_list), 'covid:%d'%valid_s[0], 'pneumonia_virus:%d'%valid_s[1], 'pneumonia_bacteria:%d'%valid_s[2]])
  csv_writer.writerow(['N of Test', len(test_list), 'covid:%d'%test_s[0], 'pneumonia_virus:%d'%test_s[1], 'pneumonia_bacteria:%d'%test_s[2]])
