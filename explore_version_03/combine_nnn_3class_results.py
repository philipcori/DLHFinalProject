...
def main():
    results_path_nnn = './explore_version_03/results_first_stage/densenet161_20200407_multiclass_cv5/result_detail_densenet161_test_cv5.csv'
    results_path_3classes = './explore_version_03/results/flannel_20200719_gamma_10_multiclass_cv5_focal/result_detail.csv'
    ...
    for (i, row_nnn) in enumerate(all_nnn_rows):
        ...
        real_labels = row_3classes[3:]
        real_labels.insert(0, row_nnn[normal_i])
        if row_nnn[0] > row_nnn[1]:     # predicted normal
            results.append([1,0,0,0] + real_labels)
        else:
            if max(row_3classes[:3]) == row_3classes[0]:
                results.append([0,1,0,0] + real_labels)
            elif max(row_3classes[:3]) == row_3classes[1]:
                results.append([0,0,1,0] + real_labels)
            else:
                results.append([0,0,0,1] + real_labels)

    with open(dest_results_dir_path, 'w') as out_file:
